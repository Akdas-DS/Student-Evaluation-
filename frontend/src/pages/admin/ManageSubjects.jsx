...
export default function ManageSubjects() {
    ...
    useEffect(() => { API.get('/admin/fields').then(r => setFields(r.data)); }, []);

    useEffect(() => {
        if (selectedSem) API.get(`/admin/subjects/${selectedSem}`).then(r => setSubjects(r.data));
        else setSubjects([]);
    }, [selectedSem]);
    ...
    const addSubject = async () => {
        ...
        await API.post('/admin/subjects', { name: newName, code: newCode, semester_id: parseInt(selectedSem) });
        setNewName(''); setNewCode('');
        API.get(`/admin/subjects/${selectedSem}`).then(r => setSubjects(r.data));
    };

    const deleteSubject = async (id) => {
        if (confirm('Delete this subject and its questions?')) {
    ...
    const addQuestion = async () => {
        ...
        API.get(`/admin/subjects/${selectedSem}`).then(r => { setSubjects(r.data); setQuestionModal(r.data.find(s => s.id === questionModal.id)); });
    };
        ...
        <div className="page-container">
            ...
            <div className="main-content">
                <div className="page-header">
                    ...
                    <p>Add subjects to semesters and configure assessment questions</p>
                </div>
                ...
                {selectedSem && (
                    <div className="glass-card card-3d" style={{ padding: 24, marginBottom: 24 }}>
                        ...
                        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                            ...
                            <input className="input-field" placeholder="Code (e.g. CS101)" value={newCode} onChange={e => setNewCode(e.target.value)} style={{ flex: 1, minWidth: 120 }} />
                    ...
                    <div key={s.id} className="glass-card fade-in" style={{ padding: 20, marginBottom: 16 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                            ...
                            <div className="inline-actions">
                                ...
                                <button className="btn btn-danger btn-sm" onClick={() => deleteSubject(s.id)}>🗑️</button>
                            </div>
                        </div>
                    </div>
                ))}
                ...
                {selectedSem && subjects.length === 0 && (
                    <div className="glass-card"><div className="empty-state"><div className="icon">📚</div><h3>No subjects in this semester</h3></div></div>
                )}
                        ...
                        <div className="modal-content glass-card">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
                                ...
                                <div style={{ display: 'flex', gap: 10 }}>
                                    ...
                                    <input className="input-field" type="number" placeholder="Weight" value={qForm.weight} onChange={e => setQForm({ ...qForm, weight: e.target.value })} style={{ width: 80 }} step="0.1" min="0.1" max="10" />
                                </div>
                                ...
                                <div key={q.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border-glass)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            ...
                                            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>weight: {q.weight}</span>
                                        </div>
                                    </div>
